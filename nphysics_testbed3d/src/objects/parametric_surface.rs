...
impl ParametricSurface {
    pub fn new<S: parametric::ParametricSurface>(collider: ColliderHandle,
                                                 ...
                                                 window:  &mut Window)
                                                 -> ParametricSurface {
        let t     = rbs.get(collider).expect("Rendering: rnot found.").position();
        ...
        */
        ...
        res.gfx.set_local_transformation(world.collider(collider).position() * res.delta);
    ...
    pub fn update(&mut self, world: &World<f32>) {
        ...
        if co.is_active() {
            self.gfx.set_local_transformation(co.position() * self.delta);

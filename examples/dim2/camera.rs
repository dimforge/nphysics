...
pub struct Camera {
    priv pressing:  bool,
    priv ui:        @mut view::View,
    priv scene:     @mut view::View,
    priv lastx:     int,
    priv lasty:     int,
    priv curr_zoom: f32
}
...
impl Camera {
    pub fn new() -> Camera {
        let scene = @mut view::View::new().unwrap();

        scene.set_center(&Vector2f::new(0.0 as f32, 0.0 as f32));

        Camera {
            pressing:  false,
            ui:        @mut view::View::new().unwrap(),
            scene:     scene,
            lastx:     0,
            lasty:     0,
            curr_zoom: 1.0
    ...
    pub fn handle_event(&mut self, event: &event::Event) {
        match *event {
            event::MouseWheelMoved(delta, _, _) => {
                let ndelta = delta as f32; // between -1.0 and 1.0

                self.curr_zoom *= (1.0 + ndelta * ZOOM_FACTOR);
                self.scene.zoom(1.0 + (ndelta * ZOOM_FACTOR) as float);
            }
            event::MouseButtonPressed(_, x, y)  => {
                self.lastx    = x;
                self.lasty    = y;
                self.pressing = true;
            }
            ...
            event::MouseMoved(x, y)             => {
                if self.pressing {
                    ...
                    self.scene.move(&Vector2f { x: zx, y: zy });
            ...
            event::Resized(w, h) => {
                self.scene.set_size(&Vector2f::new(self.curr_zoom * w as f32, self.curr_zoom * h as f32));
                self.ui.set_size(&Vector2f::new(self.curr_zoom * w as f32, self.curr_zoom * h as f32));
            }
            _ => {}
        }
    }
